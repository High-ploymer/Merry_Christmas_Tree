...
export const Experience: React.FC<ExperienceProps> = ({
  ...
  useFrame((_, delta) => {
    if (controlsRef.current) {
      ...
      // Smoothly update zoom/distance
      const currentDistance = controls.getDistance();
      const targetDistance = zoomLevel;
      const zoomLerpSpeed = 5;
      const newDistance = currentDistance + (targetDistance - currentDistance) * delta * zoomLerpSpeed;
      ...
      if (handPosition.detected) {
        ...
        // When hand is not detected, only update zoom
        const currentPolar = controls.getPolarAngle();
        const currentAzimuth = controls.getAzimuthalAngle();
        const targetY = 4;

        const x = newDistance * Math.sin(currentPolar) * Math.sin(currentAzimuth);
        const y = targetY + newDistance * Math.cos(currentPolar);
        const z = newDistance * Math.sin(currentPolar) * Math.cos(currentAzimuth);
