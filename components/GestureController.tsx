...
interface GestureControllerProps {
  ...
  lockMode?: boolean; // Prevents mode changes when true (e.g., during fireworks)
}

export const GestureController: React.FC<GestureControllerProps> = ({ onModeChange, currentMode, onHandPosition, onTwoHandsDetected, lockMode = false }) => {
  ...
  useEffect(() => {
    ...
    const startWebcam = async () => {
      ...
      if (!isSecure) {
        console.warn('⚠️ 非HTTPS环境：摄像头权限将被浏览器阻止');
        setGestureStatus("HTTPS Required - Please use https://");
        alert('⚠️ 安全限制：\n\n浏览器要求HTTPS才能访问摄像头。\n\n请确保使用以下地址：\n• https://localhost:3010\n• https://[您的IP]:3010\n\n例如：https://10.150.83.182:3010');
        return;
      }
      ...
      if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        try {
          ...
          alert('摄像头权限被拒绝。请在浏览器设置中允许摄像头访问，然后刷新页面重试。');
        }
      } else {
    ...
    const detectGesture = (landmarks: any[]) => {
      ...
      if (extendedFingers >= 4) {
        ...
        if (openFrames.current > CONFIDENCE_THRESHOLD) {
            if (lastModeRef.current !== TreeMode.CHAOS && !lockMode) {
        ...
        if (closedFrames.current > CONFIDENCE_THRESHOLD) {
            if (lastModeRef.current !== TreeMode.FORMED && !lockMode) {
